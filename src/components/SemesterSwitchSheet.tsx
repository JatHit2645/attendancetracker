...
export default function SemesterSwitchSheet({ visible, onClose, onSwitch }: SemesterSwitchSheetProps) {
  ...
  const loadSemesters = async () => {
    const { data } = await (supabase as any).from('academic_semesters').select('*').order('created_at', { ascending: false });
  ...
  const handleSelect = async (sem: any) => {
    ...
    // Use the robust DatabaseService method to prevent race conditions
          ...
          <ScrollView style={styles.list}>
            {semesters.map((sem) => (
              <TouchableOpacity
                ...
                style={[
                  styles.item, 
                  sem.is_active && styles.itemActive,
                  sem.is_active && shadow.glow(accent.primary)
                ]}
...
const styles = StyleSheet.create({
  ...
  handleIndicator: { backgroundColor: border.subtle, width: 36, height: 4, borderRadius: radius.full },
  ...
  closeButton: { backgroundColor: glass.subtle, width: 32, height: 32, borderRadius: radius.full, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: border.default },
  list: { maxHeight: 300 },
  item: { padding: spacing.lg, borderRadius: radius.lg, backgroundColor: glass.light, marginBottom: spacing.md, borderWidth: 1, borderColor: border.default },
