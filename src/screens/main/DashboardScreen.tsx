...
export default function DashboardScreen() {
    ...
    if (activeTimer) {
      try {
        await DatabaseService.logAttendanceSession({
          ...
          date: new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' }), // Fixes IST timezone bug
          status: 'present',
          ist_start_time: start.toLocaleTimeString('en-US', { hour12: false, timeZone: 'Asia/Kolkata' }),
          ist_end_time: end.toLocaleTimeString('en-US', { hour12: false, timeZone: 'Asia/Kolkata' }),
  ...
  const todayDayOfWeek = useMemo(() => {
    ...
    return new Date(istStr).getDay();
  }, []);
  const todayDateStr = useMemo(() => new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' }), []);
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        >
          <View style={{}} />
          <Text style={{}}>{activeSemester ? activeSemester.name : 'Loading Semester...'}</Text>
        </TouchableOpacity>
      </View>
